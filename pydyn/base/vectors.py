...
class Vector(VectorExpr, ABC): # 实例化向量
    ...
    def delta(self):
        if self.isOnes or self.isZero or self.isConstant:
            return Vector(s='0', size=self.size, value=np.zeros(3), attr=['Constant', 'Zero'])
        else:
...
class S2(Vector, Manifold, ABC): # q
    ...
    def delta(self):
        ...
        return Hat(self.get_variation_vector())*self
    ...
    def diff(self):
        from pydyn.operations.geometry import Hat
        return Hat(self.get_tangent_vector())*self


ZeroVector = Vector(s='0', attr=['Constant', 'Zero'])
